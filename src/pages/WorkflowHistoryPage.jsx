    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            try {
                ...
                // Force cabinet to "09 Frota"
                const targetCab = cabList.find(c => 
                    (c.Name || '').includes('09') || 
                    (c.Name || '').includes('9 /') ||
                    (c.Name || '').toLowerCase().includes('frota')
                );
                const targetCabinetId = targetCab ? targetCab.Id : (cabList[0]?.Id || '02a63cd1-672e-4c56-ad4b-bf2a7395cfd3');
    ...
    useEffect(() => {
        ...
        const loadCabinetMetadata = async () => {
            try {
                ...
                // 1. Detect Título / Document Type field
                const titleKeywords = ['titulo', 'title', 'título'];
                const typeKeywords = ['tipo', 'type', 'documento', 'doc_type', 'docclass'];
                
                let detectedTypeField = textFields.find(f => {
                    const name = (f.DBFieldName || f.FieldName || '').toLowerCase();
                    const disp = (f.DisplayName || '').toLowerCase();
                    return titleKeywords.some(kw => name === kw || disp === kw);
                });

                if (!detectedTypeField) {
                    detectedTypeField = textFields.find(f => {
                        const name = (f.DBFieldName || f.FieldName || '').toLowerCase();
                        const disp = (f.DisplayName || '').toLowerCase();
                        return titleKeywords.some(kw => name.includes(kw) || disp.includes(kw));
                    });
                }
                ...
                if (!detectedTypeField) {
                    detectedTypeField = textFields[0];
                }
                ...
                let docType = 'Pedido de Transporte';
                if (detectedTypeField) {
                    try {
                        ...
                        if (sortedValues.includes('Pedido de Transporte')) {
                            docType = 'Pedido de Transporte';
                        } else if (sortedValues.length > 0) {
                            docType = sortedValues.includes('Pedido de Transporte') ? 'Pedido de Transporte' : sortedValues[0];
                        }
                    } catch (err) {
                        console.error('Error fetching document type list:', err);
                        setTypeSuggestions(['Pedido de Transporte']);
                    }
                } else {
                    setTypeSuggestions(['Pedido de Transporte']);
                }
                ...
                <div className="card-body p-6">
                    <form onSubmit={handleSearchDocuments} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        ...
                        <div className="flex flex-wrap items-center gap-6 flex-1">
                            ...
                            {/* Título */}
                            <div className="flex items-center gap-2">
                                <span className="text-sm font-semibold text-slate-600 whitespace-nowrap">Título:</span>
                                <select
